  ...
  Marquee({
    ...
    this.onDone,
        ...
        assert(fadingEdgeStartFraction >= 0 && fadingEdgeStartFraction <= 1,
            "The fadingEdgeStartFraction value should be between 0 and 1, inclusive"),
        assert(fadingEdgeEndFraction >= 0 && fadingEdgeEndFraction <= 1,
            "The fadingEdgeEndFraction value should be between 0 and 1, inclusive"),
...
class _MarqueeState extends State<Marquee> with SingleTickerProviderStateMixin {
  ...
  void _initialize(BuildContext context) {
    ...
    assert(_totalDuration > Duration.zero,
        "With the given values, the total duration for one round would be negative. As time travel isn't invented yet, this shouldn't happen.");
  ...
  Future<void> _decelerate() async {
    await _animateTo(_decelerationTarget, _decelerationDuration,
  ...
  Widget build(BuildContext context) {
    ...
    Widget marquee = ListView.builder(
      ...
      itemBuilder: (_, i) {
        final textScaler = TextScaler.linear(widget.textScaleFactor ?? 1.0);
        final text = i.isEven
            ? Text(widget.text, style: widget.style, textScaler: textScaler)
            : _buildBlankSpace();
